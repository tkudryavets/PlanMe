...
import { CheckAuth } from './util/guards/auth.guard';
...
const routes: Routes = [
  ...
  },
  {
    ...
    loadChildren: () => import('app/components/pages/calendar/calendar.module').then(m => m.CalendarModule),
    //canActivate: [CheckAuth]
  },
  {
    path: PageRefConst.STATISTICS.name,
    loadChildren: () => import('app/components/pages/statistics/statistics.module').then(m => m.StatisticsModule),
    //canActivate: [CheckAuth]
  }
];
